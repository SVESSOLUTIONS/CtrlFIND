...
import {onOrderPayment} from '../../store/reducers/checkout';
import {useImmer} from 'use-immer';
import images from '../../constants/images';
import {translate} from '../../multiLang/translation';
export const ProductPayment = ({navigation, route}) => {
  const {user} = useContext(AuthContext);
  const dispatch = useDispatch();
  const {loading: load} = useSelector(state => state.checkout);
  ...
  const onPayPress = async () => {
    ...
    if (paymentIntent) {
      dispatch(onOrderPayment(order?.id));
    }
  };
