...
import {finishTransaction, requestPurchase} from "react-native-iap";
...
export const SubscriptionsDetailsScreen = ({navigation, route}) => {
    ...
    const subscribe_package = async (item) => {
        setLoading(false)
        try {
    ...
    const handlePurchase = async (productId, item) => {
        ...
        try {
            ...
            await finishTransaction({purchase: purchase, isConsumable: true, item})
            await subscribe_package(item)
        } catch (error) {
                ...
                <ScrollView style={{flex: 1}} showsVerticalScrollIndicator={false}>
                    ...
                    <AppButton
                        ...
                        onPress={() => {
                            if (Platform.OS === 'ios') {
                                if (item.id === 9) {
                                    subscribe_package(item)
                                } else {
                                    handlePurchase(item?.productId, item)
                                }
