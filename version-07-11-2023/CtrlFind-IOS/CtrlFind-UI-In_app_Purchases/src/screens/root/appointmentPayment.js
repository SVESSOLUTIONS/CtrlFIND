...
import {
  ...
  onAppointmentPayment,
} from '../../store/reducers/appointments';
import {useImmer} from 'use-immer';
import images from '../../constants/images';
import {translate} from '../../multiLang/translation';

export const AppointmentPayment = ({navigation}) => {
  const {user} = useContext(AuthContext);
  const dispatch = useDispatch();
  const {appointment_initial_values, loading: load} = useSelector(
  ...
  const gettingClientSecret = async () => {
    ...
    const response = await server.getClientSecret({
      amount: appointment_initial_values?.price,
    });
  ...
  const onPayPress = async () => {
    ...
    if (error) {
      return Alert.alert(
        translate("Paymentconfirmationerror"),
        error?.message ? error.message : '',
      );
    } else if (paymentIntent) {
      if (appointment_initial_values?.require_appointment === 'Y') {
        dispatch(onAppointmentPayment(appointment_initial_values));
      } else {
        dispatch(createAppointmentInstant(appointment_initial_values));
      }
    }
  };
  ...
  return (
    <>
      <AppHeader title={translate('appointment')} />
      <BaseView
