...
const islimitExceeed = (subscribe, type, count = 0) => {
  ...
  console.log("subscribe?.[`allowed_${type}`]",subscribe?.[`allowed_${type}`])
  if (subscribe && type) {
...
const getTotalAmoutWithCoupon = (items, discount, type) => {
  ...
  const discountAmount =
    discount === undefined
      ...
      ? discount
      : (totalAmount * discount) / 100;
  return (totalAmount - discountAmount).toFixed(3);
};
...
const calculateTax = (items, TaxPercentage, discount, type) => {
  ...
  //   return Math.max(0, decimalPart - exponentialPart);
  // }
  //
  // const decimalPlaces = getDecimalPlaces(totalTax);
  //
  ...
  //
  //
  // const truncatedNumber = truncateNumber(totalTax, decimalPlaces);
  // console.log(truncatedNumber); // Output: 1.99
